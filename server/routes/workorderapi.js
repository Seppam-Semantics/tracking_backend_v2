...
router.get('/RejTypeLoss_BO', (req, res, next) => {
    try {
        ...
        Query = `select sum(losses) as losses from rej_type_master rtm join rej_type_master_line rtml on rtm.id = rtml.rejTypeId WHERE rtml.color LIKE '%${color}%';`
...
router.get('/PODetailsLoss_BO', (req, res, next) => {
    try {
        ...
        Query = `SELECT pml.popl AS popl FROM po_master pm JOIN po_master_line pml ON pm.id = pml.orderId
                WHERE pm.buyer LIKE '%' || '${buyer}' || '%'
                AND pm.orderNo LIKE '%' || '${order}' || '%'
                AND pml.style LIKE '%' || '${style}' || '%'
                AND pml.color LIKE '%' || '${color}' || '%'
                AND pml.size LIKE '%' || '${size}' || '%';`
...
router.get('/ColorLoss_BO', (req, res, next) => {
    try {
        ...
        Query = `select sum(dye_process_loss)  as DyeProcessLoss from colors where color LIKE '%${color}%' ;`
