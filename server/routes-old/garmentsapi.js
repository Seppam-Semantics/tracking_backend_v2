...
router.get('/sewing-input', (req, res, next) => {
    try {
        ...
        Query = `select id, date_format(cutDate, '%d-%m-%Y') as inputDate, buyer, orderNo, style, color, size, lineNo, woId, inputPcs, bundleNo, cutId, 
...
router.get('/sewing-packing', (req, res, next) => {
    try {
        ...
        Query = `select id, date_format(packDate, '%d-%m-%Y') as packDate, buyer, orderNo, style, color, size, lineNo, woId, cutId, inputId, outputId,
                 mcUsed, mpUsed, mcHrs, packPcs, cacrtonbox, timeperiod, bundleNo
...
router.put('/sewing-packing/:id', (req, res, next) => {
    try {
        ...
        client.executeNonQuery('pput_sewing_packing(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [id, woId, buyer, orderNo, style, color, size, cutId, inputId, outputId, packDate, mcUsed, mpUsed, mcHrs, packPcs, cartonbox, bundleNo, timeperiod, loginId, orgId],
            req, res, next, function (result) {
                try {
                    rows = result;
                    if (result.success == false) {
...
router.post('/sewing-packing', (req, res, next) => {
    try {
        ...
        var packDate = req.body.packDate;
        var data = [];
        var headerQuery = "INSERT INTO tmp_sewing_packing(line_id, buyer, orderNo, style, color, size, woId, cutId, inputId, outputId, packDate, mcUsed, mpUsed, mcHrs, packPcs, cartonbox, timeperiod, bundleNo, createdBy, orgId) values "
        var data = req.body.data;
        var i = 0;
        for (let datalist of data) {
...
router.get('/sewing-shipping', (req, res, next) => {
    try {
        ...
        Query = `select id, date_format(shipDate, '%d-%m-%Y') as shipDate, buyer, orderNo, totalOrderPcs, totalshipPcs, totalPending, totalcarton, status
...
router.post('/sewing-shipping', (req, res, next) => {
    try {
        ...
        var data = [];
        var headerQuery = "INSERT INTO tmp_sewing_shipping(line_id, headId, fabtype, fabGSM, style, color, size, woId, cutId, inputId, outputId, packId, orderPcs, shipPcs, pendingPcs, carton, createdBy, orgId) values "
        var data = req.body.data;
        var i = 0;
        for (let datalist of data) {
...
router.get('/sewing-invoice', (req, res, next) => {
    try {
        ...
        Query = `select id, date_format(invoiceDate, '%d-%m-%Y') as invoiceDate, buyer, invoice, totalOrderPcs, totalvalue, status
                 from sewing_invoice_head where orgId = ${orgId} and delStatus = 0`
